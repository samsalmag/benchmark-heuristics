...
public class DeferMockMakersClassLoadingTest {
    ...
    public void mockito_should_not_load_mock_makers_it_does_not_need() throws Exception {
        ClassLoader classLoader_without_mockMakers = ClassLoaders.excludingClassLoader().withCodeSourceUrlOf(Mockito.class, Matcher.class, CustomMockMaker.class, Assertions.class).withCodeSourceUrlOf(coverageTool()).without(ByteBuddyMockMaker.class.getName(), SubclassByteBuddyMockMaker.class.getName(), InlineByteBuddyMockMaker.class.getName(), ProxyMockMaker.class.getName()).build();
    ...
    public static void createMock() {
        Assertions.assertThat(Mockito.mock(Object.class, withSettings().mockMaker(CustomMockMaker.class.getName()))).isSameAs(MY_MOCK);
    }
