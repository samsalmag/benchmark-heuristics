...
public class ReflectionMatchersTest extends TestBase {
    ...
    public void shouldNotMatchWhenFieldValuesDiffer() throws Exception {
        ...
        assertThatThrownBy(() -> {
            verify(mock).run(refEq(wanted));
        }).isInstanceOf(ArgumentsAreDifferent.class).hasMessageContainingAll("Argument(s) are different! Wanted:", "mockMe.run(", "    refEq(org.mockitousage.matchers.ReflectionMatchersTest", "Actual invocations have different arguments:", "mockMe.run(", "    org.mockitousage.matchers.ReflectionMatchersTest");
    }
