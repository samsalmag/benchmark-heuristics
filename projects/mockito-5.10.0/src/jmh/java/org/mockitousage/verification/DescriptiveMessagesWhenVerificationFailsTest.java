...
public class DescriptiveMessagesWhenVerificationFailsTest extends TestBase {
    ...
    public void should_print_fully_qualified_name_when_some_arguments_classes_have_same_simple_name() {
        ...
        String stringifiedDateArg = dateArg.toString();
        try {
            mock.overloadedMethodWithSameClassNameArguments(dateArg, "string", new IMethods.Date(0));
            verify(mock).overloadedMethodWithSameClassNameArguments(new IMethods.Date(0), "string", dateArg);
            fail();
        } catch (Throwable e) {
            String wanted = String.format("\n" + "Argument(s) are different! Wanted:" + "\n" + "iMethods.overloadedMethodWithSameClassNameArguments(" + "\n" + "    (org.mockitousage.IMethods.Date) 0," + "\n" + "    \"string\"," + "\n" + "    (java.sql.Date) %s" + "\n" + ");", stringifiedDateArg);
            assertThat(e).hasMessageContaining(wanted);
    ...
    public void test1() {
        ...
        verify(m).foo(1);
        verify(m).foo(2);
        // XXX: doesn't mention the parameters of foo(1) and foo(2)
        verify(m).foo(3);
        verify(m).foo(4);
    }
    ...
    public void test2() {
        ...
        verify(m).foo(1);
        verify(m).foo(2);
        // XXX: doesn't mention foo(4) at all
        verify(m).foo(5);
    }
