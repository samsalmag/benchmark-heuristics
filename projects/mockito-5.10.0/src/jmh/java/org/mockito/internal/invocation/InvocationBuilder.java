...
public class InvocationBuilder {
    ...
    public Invocation toInvocation() {
        if (method == null) {
            ...
            try {
                method = mockClass.getMethod(methodName, argTypes.toArray(new Class[argTypes.size()]));
            } catch (Exception e) {
                throw new RuntimeException("builder only creates invocations of IMethods interface", e);
            }
        }
        Invocation i = createInvocation(new MockStrongReference<Object>(mock, false), new SerializableMethod(method), args, NO_OP, location == null ? LocationFactory.create() : location, 1);
        if (verified) {
            i.markVerified();
        }
        return i;
    ...
    protected Invocation createInvocation(MockReference<Object> mockRef, MockitoMethod mockitoMethod, Object[] arguments, RealMethod realMethod, Location location, int sequenceNumber) {
