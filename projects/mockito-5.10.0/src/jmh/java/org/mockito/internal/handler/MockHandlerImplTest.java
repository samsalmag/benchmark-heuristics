...
public class MockHandlerImplTest extends TestBase {
    ...
    public void should_throw_mockito_exception_when_invocation_handler_throws_anything() {
        ...
        }).isInstanceOf(MockitoException.class).hasMessageContainingAll("Checked exception is invalid for this method!", "Invalid: java.lang.Throwable");
    }
    ...
    public void should_report_bogus_default_answer() throws Throwable {
        ...
        Invocation invocation = new InvocationBuilder().method(Object.class.getDeclaredMethod("toString")).toInvocation();
        // when / then
        assertThatThrownBy(() -> {
            // otherwise cast is not done
            @SuppressWarnings("unused")
            String there_should_not_be_a_CCE_here = (String) handler.handle(invocation);
        }).isInstanceOf(WrongTypeOfReturnValue.class).hasMessageContainingAll("Default answer returned a result with the wrong type:", "AWrongType cannot be returned by toString()", "toString() should return String", "The default answer of iMethods that was configured on the mock is probably incorrectly implemented.");
    }
