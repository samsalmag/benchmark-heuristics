...
class ProjectReactorValidatorTest {
    ...
    private static Stream<String> validKeys() {
        return Stream.of("foo", "123foo", "foo123", "1Z3", "a123", "123a", "1:2", "3-3", "-:", "Foobar2", "foo.bar", "foo-bar", "foo:bar", "foo_bar");
    }

    @Test
    void fail_when_invalid_key() {
        ProjectReactor reactor = createProjectReactor("foo$bar");
        assertThatThrownBy(() -> underTest.validate(reactor)).isInstanceOf(MessageException.class).hasMessageContaining("\"foo$bar\" is not a valid project key. Allowed characters are alphanumeric," + " '-', '_', '.' and ':', with at least one non-digit.");
    }

    @Test
    void fail_when_only_digits() {
        ProjectReactor reactor = createProjectReactor("12345");
        assertThatThrownBy(() -> underTest.validate(reactor)).isInstanceOf(MessageException.class).hasMessageContaining("\"12345\" is not a valid project key. Allowed characters are alphanumeric, " + "'-', '_', '.' and ':', with at least one non-digit.");
    }
    ...
    void fail_when_branch_name_is_specified_but_branch_plugin_not_present() {
        ...
        assertThatThrownBy(() -> underTest.validate(reactor)).isInstanceOf(MessageException.class).hasMessageContaining(format("To use the property \"sonar.branch.name\" and analyze branches, Developer Edition or above is required. See %s for more information.", LINK_TO_DOC));
    }
    ...
    void fail_when_pull_request_id_specified_but_branch_plugin_not_present() {
        ...
        assertThatThrownBy(() -> underTest.validate(reactor)).isInstanceOf(MessageException.class).hasMessageContaining(format("To use the property \"sonar.pullrequest.key\" and analyze pull requests, Developer Edition or above is required. See %s for more information.", LINK_TO_DOC));
    }
    ...
    void fail_when_pull_request_branch_is_specified_but_branch_plugin_not_present() {
        ...
        assertThatThrownBy(() -> underTest.validate(reactor)).isInstanceOf(MessageException.class).hasMessageContaining(format("To use the property \"sonar.pullrequest.branch\" and analyze pull requests, Developer Edition or above is required. See %s for more information.", LINK_TO_DOC));
    }
    ...
    void fail_when_pull_request_base_specified_but_branch_plugin_not_present() {
        ...
        assertThatThrownBy(() -> underTest.validate(reactor)).isInstanceOf(MessageException.class).hasMessageContaining(format("To use the property \"sonar.pullrequest.base\" and analyze pull requests, Developer Edition or above is required. See %s for more information.", LINK_TO_DOC));
    }
    ...
    private static Stream<String> validVersions() {
        return Stream.of(null, "1.0", "2017-10-16", randomAscii(100));
    }
